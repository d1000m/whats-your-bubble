    ...
    if (config.features.hoverBehavior == "dim") {
		...
		if(nodes.indexOf(e.source)<0 && nodes.indexOf(e.target)<0){
			...
			if(!neighbors[n.id]){
                ...
                n.label = n.attr['grey'] ? n.attr['true_label'] : n.label;
				n.attr['grey'] = 0;
			}
		}).draw(2,2,2);
...
function nodeActive(a) {
    ...
    if (f.attributes) {
        ...
        if (image_attribute) {
        	...
        	$GP.info_name.html("<div><span onmouseover=\"sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex['" + b.id + '\'])" onmouseout="sigInst.refresh()"><h1>' + b.label + '</span></h1>');
        }
